        ...
        <div className='mb-16'>
            <div className='flex flex-col font-normal text-[1.4rem] font-host-grotesk gap-y-[1.6rem] mb-[4rem]'>
                ...
                {contentNavigation.map(el => (
                    <Link href={`#${el.tag}-mobile`} className='cursor-pointer' key={el.tag}>{el.content}</Link>
                ))}
            </div>
            {paragraphs.map((paragraph: StrapiParagraph) => (
                <div key={`${paragraph.id}-mobile`} id={`${paragraph.tag}-mobile`} className={`flex flex-col ${paragraph.alternativeStyle ? 'font-black text-[3.2rem] font-dharma-gothic-e text-orange uppercase leading-[2.6rem]' : 'font-normal text-[1.4rem] font-host-grotesk'} gap-y-[1.6rem] mb-[4rem]`} dangerouslySetInnerHTML={{__html: paragraph.content[0].children[0].text}}></div>
            ))}
        </div>
    );
};