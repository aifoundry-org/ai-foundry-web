    ...
    return (
        <div className={`pb-12 ${className}`}>
            ...
            <div className='flex flex-row items-center justify-between mb-[7vw]'>
                <img src={imageUrl} className='w-full border-2 border-black rounded-lg' />
            </div>
            <div className='flex flex-wrap gap-2 pb-10'>
                {tags.map((tag: string, idx: number) => (
                    <div key={idx} className='bg-orange rounded-md px-[2.15vw] py-[3.2vw] md:px-[1.2vw] md:py-[1.2vw]'>
            ...
            <div className='font-host-grotesk border-b-2 border-neutral-900 flex flex-col justify-between py-4 font-paralucent gap-2'>
                ...
                <div className='flex flex-grow gap-4 text-neutral-900 items-center font-normal text-[3.75vw] leading-[5.6vw] my-[3vw]'>
                    <span className='flex items-center'>{getReadTime('')}</span>
                    <div className='bg-neutral-900 w-[1px] h-6' />
                    <div className='flex items-center gap-4'>
                        <span className='flex mr-[3vw]'>Share:</span>
                        <div className='flex flex-row justify-between items-center gap-11'>
                            <SocialShareLink className='w-[6.4vw] h-[6.4vw]' platform='instagram' title={title} articleContent={'test'} />
                            <SocialShareLink className='w-[6.4vw] h-[6.4vw]' platform='linkedin' title={title} articleContent={'test'} />
                            <SocialShareLink className='w-[6.4vw] h-[6.4vw]' platform='x' title={title} articleContent={'test'} />
                        </div>
                    </div>
                </div>
            </div>
